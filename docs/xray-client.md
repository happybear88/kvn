...
- Инструкции на этой странице рассчитаны на приложения, которые могут подключаться к серверу XRAY по протоколу VLESS с поддержкой XTLS Reality. 
...
В общем случае полную конфигурацию составляют настройки приложения, правила маршрутизации, ссылки на геофайлы и список серверов. Если что-то отсутствует, всегда можно добавить отдельно.

1. Сохраните файл [!v2rayNG_base.zip](/files/!v2rayNG_base.zip) в папку Загрузки.
2. Меню ≡ - **Резервное копирование** - **Восстановление конфигурации** - **Локально** и выберите сохраненный ZIP-файл.
...
<img src="images/v2rayn-groups.png" alt="Screenshot">
...
<img src="images/v2rayng-manage-state.png" alt="screenshot">
...
2. Меню ≡ - **Маршрутизация** - найдите правило **Избранные приложения - прокси** в конце списка - перейдите в режим редактирования, нажав на карандаш ✏️. В правиле нажмите значок, который на картинке↓ обозначен зеленой стрелкой, и добавьте в список свои приложения. Затем сохраните правило, как показывает синяя стрелка.
...
## Android Nekobox
Инструкции на этой странице подразумевают импорт конфигурации в приложение из файла JSON. В конфигурацию входит:
...
В конфигурацию также может входить подписка на серверы, и тогда вам не нужно ничего добавлять вручную. Но эта возможность имеется в любом случае.
...
1. Сохраните [этот](/files/!nekobox_settings_routing.json) или присланный вам лично файл JSON, например, в папке Загрузки.
2. Меню ≡ - **Инструменты** - **Бэкап** - **Импортировать из файла** - выберите сохраненный файл и согласитесь на перезапись всех настроек.

<img src="images/nekobox-import.png" alt="screenshot">

### Импорт серверов в Nekobox с помощью ссылки

**Примечание**. Если после импорта конфигурации на главном экране появился список серверов, ваша конфигурация сразу идет с подпиской. Вручную добавлять ничего не нужно. 
1. Скопируйте присланную ссылку вида VLESS:// в буфер обмена.
2. В приложении нажмите ➕, затем **Импорт из буфера обмена**.
...
<img src="images/nekobox-manage-state.png" alt="screenshot">
...
Чтобы только выбранные приложения ходили в обход блокировок:
1. Меню ≡ - **Настройки** - **Режим VPN для приложений** - **Прокси**: выделен (нажать). Затем поиском найдите и включите приложения, которые должны ходить через сервер.
2. Меню ≡ - **Настройки** - **Маршруты** - найдите правило **Избранные приложения - прокси** в конце списка - перейдите в режим редактирования, нажав на карандаш ✏️. В списке нажмите **Приложения** и добавьте в список свои.

<img src="images/nekobox-app-rules.png" alt="screenshot">

### Обновление подписки в Nekobox
Если вам не предоставили ссылку для подписки, то и обновлять нечего.
1. Временно отключитесь от VPN. Иначе обновление может не сработать с вашим набором правил.
2. Меню ≡ - **Настройки** - **Группы** - **Обновить**. Должно появиться информативное сообщение о добавленных / удаленных серверах, либо "Не отличается", т.е. изменений нет. 

Чтобы настроить автоматическое обновление подписки, перейдите в Меню ≡ - **Настройки** - **Группы** - нажмите ✏️ для редактирования. В настройках внизу списка включите параметры:
- **Автоматическое обновление** 
- **Задержка автоматического обновления**: 1440 минут
...
<img src="images/v2rayn-subs.png" alt="screenshot">
...
<img src="images/v2rayn-subs-update.png" alt="screenshot">
...
<img src="images/v2rayn-firefox--proxy.png" alt="screenshot">
...
<img src="images/v2rayn-proxy.png" alt="screenshot">
...
Набор правил слегка отличается от того, что используется на Android. Подразумевается использование набора геофайлов, которое задано в программе по умолчанию - от [LoyalSoldier](https://github.com/Loyalsoldier/v2ray-rules-dat).

1. Скопируйте эти правила в буфер обмена:
```
[{"outboundTag":"direct","protocol":["bittorent"],"enabled":true,"remarks":"Торрент - напрямую"},{"port":"","outboundTag":"direct","ip":["geoip:private"],"domain":["geosite:private"],"enabled":true,"remarks":"Частные сети и домены - напрямую"},{"port":"","outboundTag":"block","domain":["geosite:category-ads-all"],"enabled":true,"remarks":"Блокировать рекламу"},{"outboundTag":"proxy","ip":["1.0.0.1","1.1.1.1","8.8.8.8","8.8.4.4"],"enabled":false,"remarks":"DNS - прокси"},{"port":"","outboundTag":"proxy","domain":["geosite:slack"],"enabled":true,"remarks":"Избранные сайты - прокси"},{"port":"","outboundTag":"direct","domain":["yandex.net","yastatic.net"],"enabled":true,"remarks":"Избранные сайты - напрямую"},{"port":"","outboundTag":"direct","domain":["geosite:microsoft"],"enabled":false,"remarks":"[Test] Microsoft - напрямую"},{"port":"","outboundTag":"direct","domain":["geosite:category-ip-geo-detect"],"enabled":true,"remarks":"Сервисы определения IP - напрямую"},{"port":"","outboundTag":"direct","ip":["geoip:ru"],"domain":["geosite:category-ru","geosite:vk","geosite:yandex","geosite:mailru","geosite:mailru-group","geosite:rutube","geosite:avito","geosite:ozon","geosite:wildberries","geosite:sber","geosite:tbank-ru","domain:ru","domain:xn--p1ai","domain:su"],"enabled":true,"remarks":"Российские домены и IP - напрямую"},{"port":"0-65535","outboundTag":"proxy","enabled":true,"remarks":"Все остальное - прокси"}]
```
...
<img src="images/v2rayn-routing-basic.png" alt="screenshot">
...
<img src="images/v2rayng-logs.png" alt="screenshot"> 
...
<img src="images/v2rayn-logs.png" alt="screenshot"> 
