...
```тут скоро будут правила```

3. Меню ≡ - `Маршрутизация` - ︙(в правом верхнем углу) - `Импорт правил из буфера обмена`. Согласитесь на удаление существующих правил.
...
- `RUv1-Всё, кроме РФ` - обходит блокировки РКН, а также позволяет посещать ресурсы, которые блокируют пользователей из РФ.
