...
## Об этом документе
Это нечто вроде личных заметок, вынесенных в паблик. Получился "еще один гайд" - поверхностный, чтобы не раздумывая копировать и вставлять команды в консоль, и достаточный для новичка. Остальное в документации: [https://xtls.github.io/ru/config/](https://xtls.github.io/ru/config/).

См. также [Первоначальная настройка VPS](vps.md). Как следствие, в некоторых командах и конфигурационных файлах данного документа может использоваться учетная запись `happybear`. 
...
1. Определите последнюю стабильную версию: [https://github.com/XTLS/Xray-core/releases/](https://github.com/XTLS/Xray-core/releases/)

2. Подставьте номер версии в команду установки вместо 26.2.6
```
...
```
4. Сгенерируйте пару ключей и shortID
```
/usr/local/bin/xray x25519 #key pair
openssl rand -hex 8 #shortid
```
5. Сгенерируйте UUID - лучше создавать свой для каждого человека пользователя или группы
```
/usr/local/bin/xray uuid #uuid
```
6. Создайте конфигурационный файл (пример ниже)
```
sudo nano /usr/local/etc/xray/config.json
```
Вставьте туда содержимое файла и сохраните `Ctrl+X`. **Примечание**. Вставка большого конфигурационного файла может не сработать, если провайдер режет объемы данных по SSH. Тогда файл можно загрузить по SFTP.
...
Здесь вариант со схемой **Сервер 1 в РФ -> Сервер 2 за рубежом**. Для разнообразия примеров используется разный транспорт: от клиентов к серверу 1 - TCP, от сервера 1 к 2 - XHTTP. Еще один зарубежный сервер настраивается аналогично. 
...
```
Пояснения
- К серверу 1 два клиента подключаются по протоколу VLESS посредством TCP, остальные клиенты добавляются аналогично
- Сервер 1 также выступает клиентом для сервера 2 за рубежом, к которому подключается по протоколу VLESS посредством XHTTP
- Правила маршрутизации блокируют торренты, рекламу, а также популярные серверы проверки IP-адресов, чтобы снизить риск выявления выходного севера
- Правила маршрутизации маркируют трафик российских IP и доменов выделенными тегами `"outboundTag": "domain-ru"` и  `"outboundTag": "ip-ru"`
...
- Весь трафик выпускается наружу без фильтрации, но можно дополнительно применять правила маршрутизации - блокировать или перенаправлять на следующий сервер, если имеется
...
1. Создайте файл `nano xray-geo.sh` , вставьте код и сохраните файл.
...
2. Вставьте код, заменив имя пользователя на свое, и сохраните: 

```
CRON_TZ=UTC
5 6 * * * /home/happybear/xray-geo.sh
```
