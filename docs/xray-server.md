...
См. также [Первоначальная настройка VPS](vps.md). Как следствие, в некоторых командах и конфигурационных файлах данного документа может использоваться учетная запись `user`. 
