...
Здесь вариант со схемой **Сервер 1 в РФ -> Сервер 2 за рубежом**. Для разнообразия клиенты подключаются к первому серверу по XTLS Vision, а первый сервер ко второум - по XHTTP. Еще один зарубежный сервер настраивается аналогично. 
